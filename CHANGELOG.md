...
## 3.15.2
### Added
* Strings logged via the Go Agent's built-in logger will have strings of the form `license_key=`*hex-string* changed to `license_key=[redacted]` before they are output, regardless of severity level, where *hex-string* means a sequence of upper- or lower-case hexadecimal digits and dots ('.'). This incorporates [PR #415](https://github.com/edwardofclt/newrelic-go-agent/pull/415).
...
* Updated support for SQL database instrumentation across the board for the Go Agent’s database integrations to more accurately extract the database table name from SQL queries. Fixes [Issue #397](https://github.com/edwardofclt/newrelic-go-agent/issues/397).
...
### Fixed
* Updated mongodb driver version to 1.5.1 to fix security issue in external dependency. Fixes [Issue #358](https://github.com/edwardofclt/newrelic-go-agent/issues/358) and [Issue #370](https://github.com/edwardofclt/newrelic-go-agent/pull/370).
...
### Added
* New integration `nrpgx` added to provide the same functionality for instrumenting Postgres database queries as the existing `nrpq` integration, but using the [pgx](https://github.com/jackc/pgx) driver instead. This only covers (at present) the use case of the `pgx` driver with the standard library `database/sql`. Fixes [Issue #142](https://github.com/edwardofclt/newrelic-go-agent/issues/142) and [Issue #292](https://github.com/edwardofclt/newrelic-go-agent/issues/292)

### Changed
* Enhanced debugging logs so that New Relic license keys are redacted from the log output. Fixes [Issue #353](https://github.com/edwardofclt/newrelic-go-agent/issues/353).
...
### Fixed
* A typographical error in the nrgrpc unit tests was fixed. Fixes [Issue #344](https://github.com/edwardofclt/newrelic-go-agent/issues/344).
...
* Added advice on the recommended usage of the `app.WaitForConnection()` method. Fixes [Issue #296](https://github.com/edwardofclt/newrelic-go-agent/issues/296)
...
### Fixed
* Replaced the NR AWS SDK V2 integration for the v3 agent with a new version that works. See the v3/integrations/nrawssdk-v2/example/main.go file for an example of how to use it. Issues [#250](https://github.com/edwardofclt/newrelic-go-agent/issues/250) and [#288](https://github.com/edwardofclt/newrelic-go-agent/issues/288) are fixed by this PR. [#309](https://github.com/edwardofclt/newrelic-go-agent/pull/309)

* Fixes issue [#221](https://github.com/edwardofclt/newrelic-go-agent/issues/221): grpc errors reported in code watched by `UnaryServerInterceptor()` or `StreamServerInterceptor()` now create error events which are reported to the UI with the error message string included.  [#317](https://github.com/edwardofclt/newrelic-go-agent/pull/317)

* Fixes documentation in `GUIDE.md` for `txn.StartExternalSegment()` to reflect the v3 usage. Thanks to @abeltay for calling this to our attention and submitting PR [#320](https://github.com/edwardofclt/newrelic-go-agent/pull/320).
...
clean up grammar. [#289](https://github.com/edwardofclt/newrelic-go-agent/issues/289)
...
found. [#287](https://github.com/edwardofclt/newrelic-go-agent/issues/287) Thanks to
...
### New Features
* Aerospike is now included on the list of recognized datastore names. Thanks @vkartik97 for your PR! [#233](https://github.com/edwardofclt/newrelic-go-agent/pull/233)
* Added support for verison 8 of go-redis. Thanks @ilmimris for adding this instrumentation! [#251](https://github.com/edwardofclt/newrelic-go-agent/pull/251)

### Changes
* Changed logging level for messages resulting from Infinite Tracing load balancing operations. These were previously logged as errors, and now they are debugging messages. [#276](https://github.com/edwardofclt/newrelic-go-agent/pull/276)

### Fixed
* When the agent is configured with `cfg.ErrorCollector.RecordPanics` set to `true`, panics would be recorded by New Relic, but stack traces would not be logged as the Go Runtime usually does. The agent now logs stack traces from within its panic handler, providing similar functionality. [#278](https://github.com/edwardofclt/newrelic-go-agent/pull/278)
...
* To keep up with the latest security protocols implemented by Amazon Web
  ...
  to find utilization data. [#249](https://github.com/edwardofclt/newrelic-go-agent/pull/249)

### Changes
* Updated the locations of our license files so that Go docs https://pkg.go.dev 
  will display our agent. Thanks @tydavis for your PR to fix this! [#254](https://github.com/edwardofclt/newrelic-go-agent/pull/254)
* Added an Open Source repo linter GitHub action that runs on push. [#262](https://github.com/edwardofclt/newrelic-go-agent/pull/262)
* Updated the README.md file to correctly show the support resources from New Relic. [#255](https://github.com/edwardofclt/newrelic-go-agent/pull/255)
...
[Transaction.SetWebRequest](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#Transaction.SetWebRequest), 
it is now possible to include a `Host` field in the 
[WebRequest](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#WebRequest)
...
* Added support for [adding custom attributes directly to
  spans](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#Segment.AddAttribute).
...
* Agent attributes added to the transaction are now also added to the root Span
  ...
  godocs](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#pkg-constants).
...
* [`nrgin.Middleware`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrgin#Middleware)
  uses
  ...
  names, use
  [`nrgin.MiddlewareHandlerTxnNames`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrgin#MiddlewareHandlerTxnNames)
  instead.
...
* Attribute `http.statusCode` has been added to external span events
  ...
  1. Using
     [`NewRoundTripper`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#NewRoundTripper)
  ...
  3. Using the new
     [`ExternalSegment.SetStatusCode`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#ExternalSegment.SetStatusCode)
     API to set the status code directly

  To exclude the `http.statusCode` attribute from span events, update your
  agent configuration like so, where `cfg` is your [`newrelic.Config`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#Config) object.
...
* If you are using any of these integrations, you must upgrade them when you
 upgrade the agent:
    * [nrlambda v1.1.0](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrlambda)
    * [nrmicro v1.1.0](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrmicro)
    * [nrnats v1.1.0](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrnats)
    * [nrstan v1.1.0](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrstan)
...
* Added support for GraphQL in two new integrations:
  * [graph-gophers/graphql-go](https://github.com/graph-gophers/graphql-go)
  with
  [v3/integrations/nrgraphgophers](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrgraphgophers).
    * [Documentation](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrgraphgophers)
    * [Example](v3/integrations/nrgraphgophers/example/main.go)
  * [graphql-go/graphql](https://github.com/graphql-go/graphql)
  with
  [v3/integrations/nrgraphqlgo](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrgraphqlgo).
    * [Documentation](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrgraphqlgo)
...
* Added database instrumentation support for
  ...
  * [Documentation](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrsnowflake)
  * [Example](v3/integrations/nrsnowflake/example/main.go)

### Changes

* When using
  [`newrelic.StartExternalSegment`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#StartExternalSegment)
  or
  [`newrelic.NewRoundTripper`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#NewRoundTripper),
...
* Added support for `v7` of [go-redis/redis](https://github.com/go-redis/redis)
  in the new [v3/integrations/nrredis-v7](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrredis-v7)
  package.
  * [Documentation](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrredis-v7)
...
* Updated Gorilla instrumentation to include request time spent in middlewares.
  ...
  [godocs
  examples](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrgorilla)
  for more details.
...
* Support for W3C Trace Context, with easy upgrade from New Relic trace context.
  ...
  [`Transaction.AcceptDistributedTraceHeaders`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#Transaction.AcceptDistributedTraceHeaders).  When calling
  [`Transaction.InsertDistributedTraceHeaders`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#Transaction.InsertDistributedTraceHeaders), the Go agent will include the
...
* Added support for [elastic/go-elasticsearch](https://github.com/elastic/go-elasticsearch)
  in the new [v3/integrations/nrelasticsearch-v7](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/integrations/nrelasticsearch-v7)
  package.
...
`"github.com/edwardofclt/newrelic-go-agent"` is unchanged.  The Go agent v3.0 code in the v3
folder has the following changes:

* A [ConfigFromEnvironment](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#ConfigFromEnvironment)
  bug has been fixed.
...
agent in `"github.com/edwardofclt/newrelic-go-agent"` is unchanged.  The Go agent v3.0 code
...
* The [ConfigFromEnvironment](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#ConfigFromEnvironment)
  ...
  will be populated and [NewApplication](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#NewApplication)
  will return an error.

* [ConfigFromEnvironment](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/v3/newrelic#ConfigFromEnvironment)
...
`[go-src-dir]/src/github.com/edwardofclt/newrelic-go-agent`. Once you have the source
...
[here](https://github.com/edwardofclt/newrelic-go-agent/issues/new) and be sure to include
the label `3.0`.
...
[this issue](https://github.com/edwardofclt/newrelic-go-agent/issues/106).  
...
* The next release of the Go Agent is expected to be a major version release
  ...
  Details available here: https://github.com/edwardofclt/newrelic-go-agent/issues/106
  We would love your feedback!

### Bug Fixes

* Fixed an issue in the
  [`nrhttprouter`](http://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrhttprouter)
...
[_integrations/nrmongo](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrmongo)
package.

  * [Example application](https://github.com/edwardofclt/newrelic-go-agent/blob/master/_integrations/nrmongo/example/main.go)
  * [Full godocs Documentation](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrmongo)
...
* Added support for adding [B3
  ...
  new
  [_integrations/nrb3](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrb3)
  package's
  [`nrb3.NewRoundTripper`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrb3#NewRoundTripper)
  like this:
...
* Fixed an issue where the
  [`nrgin`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrgin/v1)
...
* Added support for a new segment type,
  [`MessageProducerSegment`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#MessageProducerSegment),
...
* Added new attribute constants for use with message consumer transactions.
  ...
  [`txn.AddAttribute`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#Transaction).
...
* A `Stack` field has been added to [Error](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#Error),
  which can be assigned using the new
  [NewStackTrace](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#NewStackTrace) function.
  ...
  [StackTracer](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#StackTracer)) in
  the error parameter, but in the error's cause as well.  This means that you
  can create an [Error](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#Error)
...
* Updated the
  [`nrmicro`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrmicro)
  package to use the new segment type
  [`MessageProducerSegment`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#MessageProducerSegment)
  and the new attribute constants:
  * [`nrmicro.ClientWrapper`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrmicro#ClientWrapper)
  ...
  * [`nrmicro.SubscriberWrapper`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrmicro#SubscriberWrapper)
    updates transaction names and adds the attribute `message.routingKey`.

* Updated the
  [`nrnats`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrnats)
  and
  [`nrstan`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrstan)
  packages to use the new segment type
  [`MessageProducerSegment`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#MessageProducerSegment)
  and the new attribute constants:
  * [`nrnats.StartPublishSegment`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrnats#StartPublishSegment)
    now starts and returns a `newrelic.MessageProducerSegment` type.
  * [`nrnats.SubWrapper`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrnats#SubWrapper)
    and
    [`nrstan.StreamingSubWrapper`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrstan#StreamingSubWrapper)
...
* Added support for [HttpRouter](https://github.com/julienschmidt/httprouter) in
  the new [_integrations/nrhttprouter](http://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrhttprouter) package.  This package allows you to easily instrument inbound requests through the HttpRouter framework.

  * [Documentation](http://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrhttprouter)
...
* Added support for [github.com/uber-go/zap](https://github.com/uber-go/zap) in
  the new
  [_integrations/nrzap](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrzap)
...
* Added new methods to expose `Transaction` details:
  ...
  * `Transaction.GetTraceMetadata()` returns a
    [TraceMetadata](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#TraceMetadata)
    which contains distributed tracing identifiers.

  * `Transaction.GetLinkingMetadata()` returns a
    [LinkingMetadata](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#LinkingMetadata)
...
* Added a new plugin for the [Logrus logging
  ...
  [_integrations/logcontext/nrlogrusplugin](https://github.com/edwardofclt/newrelic-go-agent/go-agent/tree/master/_integrations/logcontext/nrlogrusplugin)
  ...
  or view the
  [example](https://github.com/edwardofclt/newrelic-go-agent/blob/master/_integrations/logcontext/nrlogrusplugin/example/main.go).

* Added support for [NATS](https://github.com/nats-io/nats.go) and [NATS Streaming](https://github.com/nats-io/stan.go)
monitoring with the new [_integrations/nrnats](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrnats) and
[_integrations/nrstan](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrstan) packages.  These packages
support instrumentation of publishers and subscribers.

  * [NATS Example](https://github.com/edwardofclt/newrelic-go-agent/blob/master/_integrations/nrnats/examples/main.go)
  * [NATS Streaming Example](https://github.com/edwardofclt/newrelic-go-agent/blob/master/_integrations/nrstan/examples/main.go)
...
[_integrations/nrmicro](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrmicro)
package.  This package supports instrumentation for servers, clients, publishers, and subscribers.

  * [Server Example](https://github.com/edwardofclt/newrelic-go-agent/blob/master/_integrations/nrmicro/example/server/server.go)
  * [Client Example](https://github.com/edwardofclt/newrelic-go-agent/blob/master/_integrations/nrmicro/example/client/client.go)
  * [Publisher and Subscriber Example](https://github.com/edwardofclt/newrelic-go-agent/blob/master/_integrations/nrmicro/example/pubsub/main.go)
  * [Full godocs Documentation](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrmicro)
...
* Added support for custom events when using
  [nrlambda](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrlambda).
...
[_integrations/nrgrpc](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrgrpc)
package.  This package supports instrumentation for servers and clients.

  * [Server Example](https://github.com/edwardofclt/newrelic-go-agent/blob/master/_integrations/nrgrpc/example/server/server.go)
  * [Client Example](https://github.com/edwardofclt/newrelic-go-agent/blob/master/_integrations/nrgrpc/example/client/client.go)

* Added new
  [ExternalSegment](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#ExternalSegment)
...
* Improve [logrus](https://github.com/sirupsen/logrus) support by introducing
  [nrlogrus.Transform](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrlogrus#Transform),
  a function which allows you to turn a
  [logrus.Logger](https://godoc.org/github.com/sirupsen/logrus#Logger) instance into a
  [newrelic.Logger](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#Logger).
  Example use:
  ...
  [nrlogrus](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrlogrus)
...
* Introduce support for databases using
  ...
  creating
  [DatastoreSegment](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#DatastoreSegment)s.

  | Database Library Supported | Integration Package |
  | ------------- | ------------- |
  | [go-sql-driver/mysql](https://github.com/go-sql-driver/mysql) | [_integrations/nrmysql](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrmysql) |
  | [lib/pq](https://github.com/lib/pq) | [_integrations/nrpq](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrpq) |
  | [mattn/go-sqlite3](https://github.com/mattn/go-sqlite3) | [_integrations/nrsqlite3](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrsqlite3) |
  ...
  [InstrumentSQLConnector](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#InstrumentSQLConnector),
  [InstrumentSQLDriver](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#InstrumentSQLDriver),
  and
  [SQLDriverSegmentBuilder](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#SQLDriverSegmentBuilder).
...
* The [http.RoundTripper](https://golang.org/pkg/net/http/#RoundTripper) returned
  by [NewRoundTripper](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#NewRoundTripper)
...
* Added support for AWS Lambda functions in the new
  ...
  * [documentation](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrlambda)
...
* Added integration support for
  ...
  * [v1 Documentation](http://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrawssdk/v1)
  * [v2 Documentation](http://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrawssdk/v2)
...
* Added support for [New Relic Browser](https://docs.newrelic.com/docs/browser)
  ...
  [`Transaction`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#Transaction)
  which returns a
  [BrowserTimingHeader](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#BrowserTimingHeader).
...
* Added support for [Echo](https://echo.labstack.com) in the new `nrecho`
  package.
  * [Documentation](http://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrecho)
...
[NewRoundTripper](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#NewRoundTripper)
and
[StartExternalSegment](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#StartExternalSegment)
is now optional:  If it is `nil`, then a `Transaction` will be looked for in the
request's context (using
[FromContext](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#FromContext)).
...
[WebRequest](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#WebRequest)).  To
use an `*http.Request` as the parameter, use the
[NewWebRequest](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#NewWebRequest)
...
* [nrgin.Transaction](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrgin/v1#Transaction)
...
[nrgin.Transaction](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrgin/v1#Transaction)
or [FromContext](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#FromContext).
These functions now work nicely together.
For example, [FromContext](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#FromContext) will return the `Transaction`
added by [nrgin.Middleware](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrgin/v1#Middleware).
...
* Added functions [NewContext](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#NewContext)
  and [FromContext](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#FromContext)
  for adding and retrieving the Transaction from a Context.  Handlers
  instrumented by
  [WrapHandle](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#WrapHandle),
  [WrapHandleFunc](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#WrapHandleFunc),
  and [nrgorilla.InstrumentRoutes](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrgorilla/v1#InstrumentRoutes)
  may use [FromContext](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#FromContext)
  on the request's context to access the Transaction.
  Thanks to @caarlos0 for the contribution!  Though [NewContext](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#NewContext)
  and [FromContext](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#FromContext)
  require Go 1.7+ (when [context](https://golang.org/pkg/context/) was added),
  [RequestWithTransactionContext](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#RequestWithTransactionContext) is always exported so that it can be used in all framework and library
  instrumentation.
...
* The `End()` functions defined on the `Segment`, `DatastoreSegment`, and
  ...
  [`newrelic.StartSegment()`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#StartSegment)
  and
  [`newrelic.StartExternalSegment()`](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#StartExternalSegment),
  respectively.
...
* Added new `RecordCustomMetric` method to [Application](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#Application).
...
* Added [ErrorAttributer](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#ErrorAttributer),
...
* Added [Error](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#Error), a type
...
* Added support for [github.com/gin-gonic/gin](https://github.com/gin-gonic/gin)
  ...
  * [Documentation](http://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrgin/v1)
...
* Added support for [gorilla/mux](http://github.com/gorilla/mux) in the new `nrgorilla`
  package.
  * [Documentation](http://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrgorilla/v1)
...
* Added support for custom error messages and stack traces.  Errors provided
  ...
  they implement [ErrorClasser](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#ErrorClasser)
  and/or [StackTracer](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#StackTracer).
...
* Added support for [pkg/errors](https://github.com/pkg/errors).  Thanks to
  ...
  * [documentation](https://godoc.org/github.com/edwardofclt/newrelic-go-agent/_integrations/nrpkgerrors)
  * [example](https://github.com/edwardofclt/newrelic-go-agent/blob/master/_integrations/nrpkgerrors/nrpkgerrors.go)
...
* If the [DatastoreSegment](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#DatastoreSegment)
...
* Added new
[DatastoreSegment](https://godoc.org/github.com/edwardofclt/newrelic-go-agent#DatastoreSegment)
